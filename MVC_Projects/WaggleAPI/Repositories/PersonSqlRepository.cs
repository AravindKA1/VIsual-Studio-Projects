...
namespace WaggleAPI.Repositories
{
    public class PersonSqlRepository : IPersonRepository
    {
        ...
        {
            db.Database.ExecuteSqlRaw("UPDATE Person SET FirstName={0}, LastName={1}, Discriminator={3} WHERE PersonID={4}", per.FirstName, per.LastName, per.Discriminator, per.PersonID);
        }
